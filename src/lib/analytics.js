...
export async function getTopPages(clientId, days = 30, device = null) {
  ...
  let query = supabase.from('events').select('url, type, session_id, time_on_page_ms').eq('client_id', clientId).in('type', ['pageview', 'time_on_page']).gte('created_at', since);
  if (device) query = query.eq('device_type', device);

  const { data: events } = await query;
  const { data: convEvents } = await supabase.from('conversion_events').select('url').eq('client_id', clientId).gte('created_at', since);
...
export async function getSources(clientId, days = 30) {
  const { since } = dateRange(days);
  const { data: sessions } = await supabase.from('sessions').select('session_id, utm_source, utm_medium, referrer_url, converted').eq('client_id', clientId).gte('created_at', since);
  if (!sessions) return [];
...
export async function getConversionPaths(clientId, days = 30) {
  const { since } = dateRange(days);
  const { data: convSessions } = await supabase.from('conversion_events').select('session_id').eq('client_id', clientId).gte('created_at', since);
