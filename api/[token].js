// api/track/[token].js — v2.1
// Cache-Control set to no-store so conversion goal changes take effect immediately.
...
export default async function handler(req, res) {
  ...
  // No caching — ensures goal changes are picked up on every page load
...
var _utms=(function(){
  ...
  try{var stored=JSON.parse(sessionStorage.getItem('__cru__')||'{}');
  ...
  window.addEventListener('scroll',function(){
    if(!_tick){requestAnimationFrame(function(){
      ...
      for(var t=5;t<=100;t+=5){
        if(!_fired[t]&&pct>=t){_fired[t]=true;
...
function _trackForms(){
  ...
  document.addEventListener('focusout',function(e){
    ...
    _enq(Object.assign(_base('form_field'),{
      ...
      field_name:el.name||el.id||null,field_type:el.type||tag,
      time_to_fill_ms:timeMs,field_index:idx>=0?idx:null,xpath:_xp(el)}));});
  document.addEventListener('submit',function(e){
    var form=e.target;
    if(!form||form.tagName.toLowerCase()!=='form')return;
    _enq(Object.assign(_base('form_submit'),{
      form_id:(form.id||form.name)||null,xpath:_xp(form)}));
    _checkGoals('form_submit',form);},{passive:true});}
var _firedGoals={};
function _checkGoals(triggerType,el){
  ...
  _goals.forEach(function(g){
    ...
    var match=false;
    if((g.type==='click'&&triggerType==='click'||g.type==='form_submit'&&triggerType==='form_submit')&&g.sel){
...
}else{_trackPV();_trackVisibility();_checkPageLoadGoals();}
document.addEventListener('click',_trackClick,{passive:true});
_trackScroll();_trackForms();
