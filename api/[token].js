...
var _FLUSH=2000,_QK='__crq_${clientId.replace(/[^a-z0-9]/g,'_')}__',_MAX=150;
...
function _base(type){
  var ev={
    event_id:_uuid(),type:type,client_id:_cid,session_id:_sid,
...
function _trackPV(){
  _enq(Object.assign(_base('pageview'),{title:document.title}));
}
...
function _trackClick(e){
  ...
  _enq(Object.assign(_base('click'),{
    ...
    viewport_y:Math.round(e.clientY),
    ...
    if(g.type==='click_url'&&triggerType==='click'){
      ...
      for(var i=0;i<5;i++){
        ...
        if(node.tagName&&node.tagName.toLowerCase()==='a'){
          href=node.href||node.getAttribute('href')||'';break;
        }
